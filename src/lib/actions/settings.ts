...
export async function updateLeaveType(id: string, input: LeaveTypeInput): Promise<{ ok: boolean; error?: string }> {
  try {
    ...
    if (error) return { ok: false, error: error.message }
    await createAuditLog({ action: 'update', entity_type: 'leave_type', entity_id: id, new_values: input })
    return { ok: true }
  } catch (e: any) { return { ok: false, error: e.message } }
}
...
export async function createCentre(input: CentreInput): Promise<{ id: string } | { error: string }> {
  try {
    ...
    if (error) return { error: error.message }
    await createAuditLog({ action: 'create', entity_type: 'centre', entity_id: data.id, new_values: input })
    return { id: data.id }
  } catch (e: any) { return { error: e.message } }
}
...
export async function updateCentre(id: string, input: CentreInput): Promise<{ ok: boolean; error?: string }> {
  try {
    ...
    if (error) return { ok: false, error: error.message }
    await createAuditLog({ action: 'update', entity_type: 'centre', entity_id: id, new_values: input })
    return { ok: true }
  } catch (e: any) { return { ok: false, error: e.message } }
}

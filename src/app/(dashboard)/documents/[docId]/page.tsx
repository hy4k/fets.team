...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex py-2.5 border-b border-[#1B2A22] last:border-0">
      <dt className="w-44 text-xs text-[#66756A] flex-shrink-0 pt-0.5">{label}</dt>
...
export default async function DocumentViewPage({ params }: Props) {
  ...
  const qrDataUrl = await QRCode.toDataURL(verifyUrl, {
    width: 160,
    margin: 2,
    color: { dark: '#0D2018', light: '#FFFFFF' },
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4">
          <Link href="/document-history" className="p-2 hover:bg-[#1B2A22] rounded-lg text-[#A3B1A5] hover:text-white transition-colors mt-0.5">
          ...
          <div>
            ...
            <p className="text-sm text-[#66756A] mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="card-glass rounded-xl p-5">
          <h3 className="text-xs font-semibold text-[#66756A] uppercase tracking-wider mb-4">Staff</h3>
          {staff ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#C9A35C] flex items-center justify-center text-black font-bold text-sm">
                  {staff.full_name.charAt(0)}
                </div>
                <div>
                  <div className="text-sm font-semibold text-white">{staff.full_name}</div>
                  <div className="text-xs text-[#66756A]">{staff.staff_id}</div>
                </div>
              </div>
              {staff.designation_text && <div className="text-xs text-[#A3B1A5]">{staff.designation_text}</div>}
              {staff.centre && <div className="text-xs text-[#66756A]">{staff.centre.name}</div>}
            </div>
          ) : (
            <p className="text-sm text-[#33473A]">No staff linked</p>
          )}
        </div>

        {/* Document info */}
        <div className="lg:col-span-2 card-glass rounded-xl p-5">
          <h3 className="text-xs font-semibold text-[#66756A] uppercase tracking-wider mb-4">Document Fields</h3>
          <dl>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 card-glass rounded-xl p-5">
          <h3 className="text-xs font-semibold text-[#66756A] uppercase tracking-wider mb-4">Metadata</h3>
        ...
        {/* QR Widget */}
        <div className="card-glass rounded-xl p-5 flex flex-col items-center justify-center gap-3">
          <h3 className="text-xs font-semibold text-[#66756A] uppercase tracking-wider self-start">Verify QR</h3>
          ...
          <p className="text-xs text-[#66756A] text-center leading-relaxed">
            Scan to verify this document&apos;s authenticity
          </p>
          <a
            href={verifyUrl} target="_blank" rel="noopener noreferrer"
            className="text-xs text-[#14B8A6] hover:text-[#5EEAD4] transition-colors truncate max-w-full"
          >
            fets.team/verify/…
          </a>
        </div>
